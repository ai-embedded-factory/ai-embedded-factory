    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for i, row in enumerate(r, start=2):
            ...
            if not req.startswith("REQ-"):
                print(f"ERROR L{i}: invalid req_id '{req}'")
                ok = False
            if st in ("APPROVED","IMPLEMENTED","VERIFIED") and (not code or not tst):
                print(f"ERROR L{i}: '{req}' status={st} requires code_ref and test_id")
                ok = False
