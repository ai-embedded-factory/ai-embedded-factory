...
int main(void) {
  /* mpx_mq_extendattr_getqueueaddr: This pthreadsmore service returns the previously supplied message queue memory address. */
  ...
  skip_if_enosys();
  if (skipped) {
    printf("SKIP (stubbed): mpx_mq_extendattr_getqueueaddr\n");
    return 0;
  }
