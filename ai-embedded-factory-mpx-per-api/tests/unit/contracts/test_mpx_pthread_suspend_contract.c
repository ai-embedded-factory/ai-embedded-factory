...
int main(void) {
  /* mpx_pthread_suspend: This pthread+ service suspends the specified thread. If the specified thread is already suspended, an error is returned. */
  ...
  skip_if_enosys();
  if (skipped) {
    printf("SKIP (stubbed): mpx_pthread_suspend\n");
    return 0;
  }
