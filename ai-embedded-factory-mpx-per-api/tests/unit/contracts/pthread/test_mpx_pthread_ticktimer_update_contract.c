 ...
 * API: mpx_pthread_ticktimer_update
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)mpx_pthread_ticktimer_update(0, 0, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
static int st_state_errors(void) {
  errno = 0;
  int rc = (int)mpx_pthread_ticktimer_update(0, 1, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
  if ((errno != EBUSY) && (errno != EINVAL)) return 1;
  return 0;
...
int main(void) {
  /* Description (extract): This pthreadsmore service updates the initial ticks and reload ticks of a previously created but stopped ticktimer. */
  ...
  errno = 0;
  (void)mpx_pthread_ticktimer_update(0, 0, 0);
  if (mpx_skip_if_stubbed("mpx_pthread_ticktimer_update")) return 0;
  ...
  printf("PASS (scaffold): mpx_pthread_ticktimer_update\n");
  return 0;
}
