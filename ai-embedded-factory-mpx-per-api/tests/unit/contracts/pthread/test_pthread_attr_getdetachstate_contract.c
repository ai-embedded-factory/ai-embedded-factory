 ...
 * API: pthread_attr_getdetachstate
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)pthread_attr_getdetachstate(0, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This service returns the detach state stored in the thread attributes structure. Valid detach states are PTHREAD_CREATE_JOINABLE (default) and PTHREAD_CREATE_DETACHED. */
  ...
  errno = 0;
  (void)pthread_attr_getdetachstate(0, 0);
  if (mpx_skip_if_stubbed("pthread_attr_getdetachstate")) return 0;
  ...
  printf("PASS (scaffold): pthread_attr_getdetachstate\n");
  return 0;
}
