 ...
 * API: mpx_pthread_ticktimer_destroy
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)mpx_pthread_ticktimer_destroy(0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This pthreadsmore service destroys a previously created but stopped ticktimer. */
   ...
   * EBUSY	Ticktimer is not stopped, i.e., it is still active.
   * Real-time Scenarios:
  ...
  if (mpx_skip_if_stubbed("mpx_pthread_ticktimer_destroy")) return 0;
  ...
  printf("PASS (scaffold): mpx_pthread_ticktimer_destroy\n");
  return 0;
}
