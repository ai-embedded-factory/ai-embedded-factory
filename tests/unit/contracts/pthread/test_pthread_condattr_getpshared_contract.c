 ...
 * API: pthread_condattr_getpshared
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)pthread_condattr_getpshared(0, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This service returns the current process sharing designation in the condition variable attribute structure. By default, the process sharing designation is PTHREAD_PROCESS_PRIVATE. */
  /* Errno symbols observed in manual extract: EINVAL */
  /* Return codes / errno notes (extract, first lines):
   * MPX_SUCCESS (0)	Successful condition variable attributes process
   * sharing designation retrieval.
  ...
  if (mpx_skip_if_stubbed("pthread_condattr_getpshared")) return 0;
  ...
  printf("PASS (scaffold): pthread_condattr_getpshared\n");
  return 0;
}
