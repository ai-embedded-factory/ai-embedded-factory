...
static int st_param_validation(void) {
  errno = 0;
  int rc = (int)pthread_cond_wait(0, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
   ...
   * EINVAL	Invalid condition variable handle or mutex handle pointer. In addition, if the specified mutex is not owned or a different mutex is
   ...
   * SUSPENSION. The calling thread is suspended until another thread signals or broadcasts to the condition variable.
  ...
  printf("PASS (scaffold): pthread_cond_wait\n");
  return 0;
}
