 ...
 * API: pthread_condattr_init
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)pthread_condattr_init(0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This service initializes the condition variable attributes structure with default condition variable creation values. These defaults are as follows: */
  ...
  if (mpx_skip_if_stubbed("pthread_condattr_init")) return 0;
  ...
  printf("PASS (scaffold): pthread_condattr_init\n");
  return 0;
}
