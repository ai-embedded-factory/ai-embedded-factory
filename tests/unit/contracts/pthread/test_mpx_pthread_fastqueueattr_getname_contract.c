 ...
 * API: mpx_pthread_fastqueueattr_getname
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)mpx_pthread_fastqueueattr_getname(0, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This pthreadsmore service returns the previously supplied fastqueue name. */
  ...
  if (mpx_skip_if_stubbed("mpx_pthread_fastqueueattr_getname")) return 0;
  ...
  printf("PASS (scaffold): mpx_pthread_fastqueueattr_getname\n");
  return 0;
}
