...
static int st_param_validation(void) {
  errno = 0;
  int rc = (int)pthread_condattr_setpshared(0, -1);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  ...
  errno = 0;
  (void)pthread_condattr_setpshared(0, -1);
  ...
  printf("PASS (scaffold): pthread_condattr_setpshared\n");
  return 0;
}
