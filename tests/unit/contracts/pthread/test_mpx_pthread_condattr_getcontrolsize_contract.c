 ...
 * API: mpx_pthread_condattr_getcontrolsize
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)mpx_pthread_condattr_getcontrolsize(0, 0);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This pthreadsmore service returns the size of the internal condition variable control structure. The primary purpose of this API is to inform the application how much memory is required for the mpx_pthread_condattr_setcontroladdr API. */
  ...
  if (mpx_skip_if_stubbed("mpx_pthread_condattr_getcontrolsize")) return 0;
  ...
  printf("PASS (scaffold): mpx_pthread_condattr_getcontrolsize\n");
  return 0;
}
