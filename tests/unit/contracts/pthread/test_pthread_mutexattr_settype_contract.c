 ...
 * API: pthread_mutexattr_settype
 */

static int st_param_validation(void) {
  errno = 0;
  int rc = (int)pthread_mutexattr_settype(0, -1);
  if (rc != -1) return 1;
  if (errno == 0) return 1;
...
int main(void) {
  /* Description (extract): This service sets the specified mutex type in the previously created attributes structure. By default, the type is PTHREAD_MUTEX_ERRORCHECK. */
  ...
  errno = 0;
  (void)pthread_mutexattr_settype(0, -1);
  if (mpx_skip_if_stubbed("pthread_mutexattr_settype")) return 0;
  ...
  printf("PASS (scaffold): pthread_mutexattr_settype\n");
  return 0;
}
